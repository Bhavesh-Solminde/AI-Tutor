...
import { Brain, ArrowRight, ShieldCheck, Zap, Calendar, MessageSquareText, Layers, Eye, EyeOff } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-slate-800 dark:text-text-primary-dark transition-colors duration-300 relative overflow-hidden flex flex-col justify-between">
      ...
      <main className="relative z-10 max-w-7xl mx-auto w-full px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center flex-1">
        ...
        <div className="lg:col-span-7 space-y-8 text-left">
          ...
          <p className="text-lg text-slate-600 dark:text-text-muted-dark max-w-xl">
            Upload your syllabus or paste your notes. NeuralNest builds an interactive roadmap, teaches topic by topic, and adapts explanations until you achieve mastery.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4">
            {[
              { icon: Layers, title: 'Roadmap Nodes', desc: 'Nodes turn from gray to yellow to green as you master topics.' },
              { icon: Calendar, title: 'Rescue Countdown', desc: 'Set your exam date and get an auto-adjusted rescue plan.' },
              { icon: MessageSquareText, title: 'Adaptive Pacing', desc: 'Comprehension checks and doubt Q&A before moving forward.' },
            ].map(({ icon: Icon, title, desc }) => (
              <div key={title} className="p-4 rounded-xl border border-border-light dark:border-border-dark bg-white dark:bg-surface-dark/50 shadow-sm space-y-2">
                <div className="text-primary dark:text-accent"><Icon className="h-5 w-5" /></div>
        ...
        <div className="lg:col-span-5 flex justify-center">
          <div className="w-full max-w-md p-8 rounded-2xl border border-border-light dark:border-border-dark bg-white dark:bg-surface-dark shadow-2xl relative">
            <div className="absolute -top-12 -left-12 w-24 h-24 bg-primary/20 dark:bg-accent/20 rounded-full blur-2xl" />
            <div className="absolute -bottom-12 -right-12 w-32 h-32 bg-purple-500/20 rounded-full blur-2xl" />

            <div className="relative z-10 space-y-6">
              <div className="text-center">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Welcome to NeuralNest</h2>
              ...
              <div className="flex border border-border-light dark:border-border-dark rounded-xl overflow-hidden p-1 bg-slate-50 dark:bg-elevated-dark">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1.5">Email Address</label>
                  <input
                    type="email"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1.5">Password</label>
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-4 py-3 pr-10 text-sm rounded-xl border border-border-light dark:border-border-dark bg-slate-50 dark:bg-elevated-dark text-slate-900 dark:text-text-primary-dark placeholder-slate-400 dark:placeholder-text-muted-dark focus:outline-none focus:border-primary dark:focus:border-primary transition-all duration-300"
                    />
                    <button
                      type="button"
      ...
      <footer className="relative z-10 max-w-7xl mx-auto w-full px-6 py-6 border-t border-border-light dark:border-border-dark flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
        <p className="text-xs text-slate-500 dark:text-slate-400">
          &copy; {new Date().getFullYear()} NeuralNest-OS.
        </p>
